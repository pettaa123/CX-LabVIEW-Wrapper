...
int32_t createPointcloud(const char* calibId,const char* calibFile,const char* pcdFile, Arr2D_U16Hdl arr) {
	try
	{
...
int32_t writeTif_U16(const char* file, Arr2D_U16Hdl arr) {
	try
	{
		if ((*arr)->dimSizes[0] <= 0 || (*arr)->dimSizes[1] <= 0)
			return EXIT_FAILURE;
...
int32_t readTif_U16(const char* file, Arr2D_U16Hdl arr) {
	try
	{
		std::string fname(file);
